...
objFuncEval <- function(my_optimized_par,
  ...
  for(cur_gene in 1:nu_genes){
    ...
    if(round_to_int){ # round the predicted expression mat
      Final_exp_discretized <- prediction_discretizer(prediction = Final_exp,
  ...
  if(round_to_int){ # compute a new error indicating the (number of true predictions) / (number of wrong predictions + number of True predictions) 
    ...
    return(list(Predicted_expression_mat=PredictedGeneExpMat_discrete,
                ...
                Predicted_expression_mat_realValue=Predicted_expression_mat,
                discretization_threshold=PredictedGeneExpMat_discrete$threshods))
  }
...
########################################################################################################################
########################################################################################################################
obj_func_chosen_enh <- function(my_par, my_data, chosen_enh_ind){
  # my_data is the output of "createOptimDataset"
  ...
  # my_par is a numeric vector of length equal to "number of TFs plus one": 
  #    one parameter per TF plus one intercept
  ...
  for(cur_gene in 1:nu_genes){
    ...
    for(cur_cond in 1:length(cur_gene_cond_inds)){
      # Current condition's control and treatment TF expressions times the affinity, concatenated with 1 for intercept 
      TF_C <- c((my_data$gene_conditon_mat[cur_gene_cond_inds[cur_cond], 1:nu_TFs] * TF_aff), 1)
      TF_T <- c((my_data$gene_conditon_mat[cur_gene_cond_inds[cur_cond], (nu_TFs+1):(2*nu_TFs)] * TF_aff), 1)
        # print("TF_T:")
        # print(TF_T)
        # print("TF_C :")
        # print(TF_C)
      ...
      cur_loss_cond <- (cur_pred - cur_obs)^2 * loss_weight[cur_gene_cond_inds[cur_cond]]
      cur_loss <- cur_loss + cur_loss_cond
    }# end of loop over conditions
...
obj_func_chosen_enh_gradient <- function(my_par, my_data, chosen_enh_ind, intercept_pergene=T, constrained=T, return_grad=T){
  # my_data is the output of "createOptimDataset"
  # chosen_enh_ind is an integer vector of length = number of genes. contains the index of
  #    the chosen enhancer among the enhancers associated with this gene 
  ...
  if(intercept_pergene){
    names(my_par) <- c(colnames(my_data$gene_conditon_mat)[1:nu_TFs],  paste("intercept", data_gene_name_unique, sep = "_"))
      ...
      if(return_grad){
        ...
        if(intercept_pergene){
          ...
          for(cur_tf in 1:nu_TFs){
            if(constrained){
              partial_control <- 2 * TF_C[cur_tf] * F_treat
              partial_treat   <- 2 * TF_T[cur_tf] * F_control
            }else{
            ...
            cur_grad <- (cur_pred - cur_obs) * loss_weight[cur_gene_cond_inds[cur_cond]] * partial_grad
...
res <- nloptr( x0=aa_start_par,
               eval_f=obj_func_chosen_enh_gradient, my_data = ER_52_opt_simAnn_2_input,chosen_enh_ind = Sim_Ann_weighted_148_restart_enhancers[1, ], intercept_pergene=T, 
         #      lb=lb,
          #     ub=ub,
...
res2 <- nloptr( x0=aa_start_par,
               eval_f=obj_func_chosen_enh_gradient, my_data = ER_52_opt_simAnn_2_input,chosen_enh_ind = Sim_Ann_weighted_148_restart_enhancers[1, ], intercept_pergene=T, constrained=F,
               #      lb=lb,
               #     ub=ub,
               #eval_g_ineq=obj_func_chosen_enh_ineq,
               #     eval_g_eq=eval_g_eq,
...
aa_sbplx <- sbplx(x0 = aa_start_par, fn = obj_func_chosen_enh_gradient, my_data = ER_52_opt_simAnn_2_input, chosen_enh_ind = Sim_Ann_weighted_148_restart_enhancers[1, ],intercept_pergene=T, constrained=F, return_grad=F)
proc.time()
...
SimAnnObjOptim <- function(.my_data,
  ...
  Result_list$parameters[[cnt]] <- First_optim$par
  Result_list$obj_value[[cnt]] <- First_optim$value
  cnt <- cnt + 1
  cnt_all <- 1
  Temp <- starting_temp
    ...
    for(cur_iter in 1:iter_per_temp){
      ...
      for(cur_gene in 1:nu_genes){
        ...
        if(nu_enh_per_gene[shuffled_gene_index[cur_gene]] > 1){
          ...
          accprob <- SimAnnAcceptProb(cur_sol = cur_optim$value,
                                      prev_sol = Result_list$obj_value[[cnt-1]],
                                      temp = Temp)
          if(accprob > runif(1)){
            Result_list$Enhancer_index[[cnt]] <- cur_chosen_enh_temp
            Result_list$parameters[[cnt]] <- cur_optim$par
            Result_list$obj_value[[cnt]] <- cur_optim$value
...
########################################################################################################################
########################################################################################################################
SimAnnStepJobConstructor <- function(script_name="ER_Optimizer_SimAnn_hal.R",
                                     nu_jobs,
...
DAGmanConstructor <- function(submit_prefix,
  ...
  # order is an integer vector of length equal to number of submit jobs, containing
  #   the order by which the jobs have to be executed. 1 being the first job. many jobs
  #   can have the same priority if needed
  # filename is the name of the file to wite to
  ...
  uniqe_prio <- unique(order)
  for(line_nu in 1:(length(uniqe_prio) - 1)){
    cur_parents <- job_ind[which(order %in% uniqe_prio[line_nu])]
    cur_childs <- job_ind[which(order %in% uniqe_prio[line_nu+1])]
...
# This analysis showed that in many cases the best enhancer was not chosen and was refined in evaluations.
...
aa_time_optim <- proc.time()
aa_optim <- optim(par = aa_start_par, fn = obj_func_chosen_enh, my_data = ER_52_opt_simAnn_2_input, chosen_enh_ind = Sim_Ann_weighted_148_restart_enhancers[1, ])
proc.time()
aa_time_sbplx <- proc.time()
aa_sbplx <- sbplx(x0 = aa_start_par, fn = obj_func_chosen_enh, my_data = ER_52_opt_simAnn_2_input, chosen_enh_ind = Sim_Ann_weighted_148_restart_enhancers[1, ])
proc.time()
